...
# searches "*.fasta" (commandline arg) and returns profiles for helices with a length of 20
...
$counter=0;
$counter_2=0;
while ($counter<$j){		#main routine: searches for helices with a length >= 20 and saves them and the corresponding sequence to a hash
...
print $out_helix "ID\tNumber_of_consecutive_helix\tAA-Sequence\n";
print $out_nonhelix "ID\tNumber_of_consecutive_non-helix\tAA-Sequence\n";

foreach my $id (keys %helices) {									#cuts helices at length 20
    foreach my $values (keys %{ $helices{$id} }) {
		foreach my $ss(keys %{ $helices{$id}{$values} }) {
			...
			$rest=(int(length($ss)/20));
			$mod=(length($ss))%20;
			$plus=0;
			for ($j=0; $j<$rest; $j++){
				$random_index = int(rand($mod));
				$ss_25 = substr($ss, $random_index+($plus), 20);
				$ss_25_seq_h = substr($a, $random_index+($plus), 20);
				$helices_25{$id}{$values}{$j}{$ss_25}=$ss_25_seq_h;
				print $out_helix "$id\t$j\t$ss_25_seq_h\n";
				$plus=20+$random_index+$plus;
...
foreach my $id (keys %nonhelices) {									#cuts non-helices at 20
    foreach my $values (keys %{ $nonhelices{$id} }) {
		foreach my $ss (keys %{ $nonhelices{$id}{$values} }) {
			...
			for ($j=0; $j<$rest; $j++){
				$random_index = int(rand($mod));
				$ss_25_nh = substr($ss, $random_index+($plus), 20);
				$ss_25_seq_nh = substr($b, $random_index+($plus), 20);
				$helices_25{$id}{$values}{$j}{$ss_25_nh}=$ss_25_seq_nh;
				print $out_nonhelix "$id\t$j\t$ss_25_seq_nh\n";
				$plus=20+$random_index+$plus;
				$mod=$mod-$random_index;
			}
		}
